...
void loop() {
  ...
  if((millis()-Time_) > TIME_SEND)
  {
    ...
    //***************************************************************************
    Serial.println("Enviando el dato con la funcion SEND!\r\n");
    // Funcion SEND: permite enviar datos pequeños al Gateway
    // La ventaja es que no añade valores hexadecimales al dato a enviar
    // Macaron.RAK3172_LoraWanSend(puerto, payload, intentos, tiempo(milisegundos))
    // envio por el puerto 2 la variable constante 100(formato Hexadecimal)
